...
class Mapper:
    def upate(self, str='', params: dict | tuple | list = None) -> bool:
        pass

    def select(self, sql: str = '', params: dict | tuple | list = None) -> tuple[list, list]:
        pass

    def qry_data_one(self, sql: str = '', params: dict | tuple | list = None) -> dict:
        pass

    def qry_data_list(self, sql: str = '', params: dict | tuple | list = None) -> list:
        columns, records = self.select(sql, params)
        return [dict(zip(columns, record)) for record in records]

    def qry_data_frame(self, sql: str = '', params: dict | tuple | list = None) -> pd.DataFrame:
        columns, records = self.select(sql, params)
        # 将结果转换为 DataFrame
...
class MyMapper(Mapper):
    ...
    def _check_onnection(self):
        try:
            logger.debug(self.qry_data_one('SELECT VERSION()'))
        except (Exception, _MyError) as error:
            logger.debug('Error while connecting to MySQL', error)
            raise error

    def update(self, sql: str = '', params: dict | tuple | list = None):
    ...
    def select(self, sql: str = '', params: dict | tuple | list = None):
        """
    ...
    def qry_data_one(self, sql: str = '', params: dict | tuple | list = None):
        """
