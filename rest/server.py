...
class RestRequest:
    def __init__(self, method: str, path: str, headers=None, body=None):
        self.method = method
        self.path = path
...
def _send_success(handler: BaseHTTPRequestHandler, response: RestResponse):
    ...
    if response.body:
        ...
        _send_msg(handler, response.code, headers=headers, body=json_.dumps(response.json, ensure_ascii=False))
    else:
...
class RestRoute:
    def __init__(self, path: str, target: object, methods: Tuple[str] = (), name=''):
        self.methods = list()
        ...
        self._args_ = inspect.getfullargspec(target).args
    ...
    def handler(self, request: RestRequest, response: RestResponse):
        try:
            ...
            logger.error('handler exec except: %s', e, exc_info=True)
            response.json = {'code': 500, 'msg': '服务异常', 'except': str(e)}
            setattr(response, 'code', HTTPStatus.OK)
...
class RestServer(ThreadingHTTPServer):
    ...
    def _handle_rest(self, handler: BaseHTTPRequestHandler):
        path, method, headers, body = handler.path, handler.command, {}, ''
        ...
        try:
            ...
            logger.error('server except: %s', e, exc_info=True)
